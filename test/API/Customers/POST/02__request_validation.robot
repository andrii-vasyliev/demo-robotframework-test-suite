...
Create customer name validation
    ...
    ${422}    cannot be empty and must contain only allowed chars    name=${EMPTY}    email=e@mail.com
    ${422}    cannot be empty and must contain only allowed chars    name=${SPACE}    email=e@mail.com
    ${422}    cannot be empty and must contain only allowed chars    name=${{ 'Some\nName' }}    email=e@mail.com
    ${422}    cannot be empty and must contain only allowed chars    name=${{ 'Some\tName' }}    email=e@mail.com
    ${422}    cannot be empty and must contain only allowed chars    name=();:{}    email=e@mail.com
...
Create customer uniqueness validation
    ...
    ${409}    Customer already exist    name=Same Name
    ${409}    Customer already exist    name=Same Name And Email    email=same@email.to


*** Keywords ***
Setup Uniqueness Validation Case
    Basic Test Setup
    API Create Customer    Same Name    ${None}
    API Create Customer    Same Name And Email    same@email.to
